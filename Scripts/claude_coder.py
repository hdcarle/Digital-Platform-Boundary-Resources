#!/usr/bin/env python3
"""
Claude Boundary Resource Coding Agent
======================================
...
Usage:
    python3 claude_coder.py scraped_content/ tracker.xlsx --output results/
    python3 claude_coder.py scraped_content/ tracker.xlsx --limit 5  # Test on 5
    python3 claude_coder.py scraped_content/ tracker.xlsx --dry-run  # Preview
...
# Auto-zero template for PLAT = NONE
AUTO_ZERO_RESULT = {
    "platform_controls": {
        "AGE": 0,
        "API_YEAR": ""
...
CODING_PROMPT = '''You are a specialized research assistant coding digital platform boundary resources for academic research on platform internationalization. You must follow the codebook rules exactly and return only valid JSON output.
...
Analyze the content above and return ONLY the JSON output.'''
...
class ClaudeBRCoder:
    """Codes boundary resources using Claude API."""
    ...
    def __init__(self, output_dir: str, verbose: bool = True, max_content_chars: int = 200000):
        ...
        # The coding prompt itself is ~15-20K tokens. At ~4 chars/token, 200K chars of content
        # ≈ 50K tokens + 20K prompt + 4K response = ~74K tokens, well within limits.
        # Previous 300K default caused failures on large platforms (e.g., KaKao at 257K chars).
        # Use --max-content flag to override if needed.
    ...
    def code_platform(self, platform_id: str, platform_name: str, plat_status: str,
        ...
        try:
            self.log(f"  Sending to Claude API ({MODEL})...")
            ...
            coding_result = json.loads(json_match)
    ...
    def code_from_tracker(self, scraped_dir: str, tracker_file: str,
        ...
        total = len(df)
        self.log(f"\n{'='*60}")
        self.log(f"CLAUDE BOUNDARY RESOURCE CODER")
        self.log(f"{'='*60}")
        ...
        for idx, (_, row) in enumerate(df.iterrows(), 1):
            ...
            result_file = self.output_dir / f"{platform_id}_claude.json"
        ...
        summary_file = self.output_dir / "claude_coding_summary.json"
    ...
    def _export_to_csv(self, results: dict):
        ...
        if rows:
            ...
            csv_file = self.output_dir / "claude_coding_results.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description='Code boundary resources using Claude API',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python3 claude_coder.py scraped_content/ tracker.xlsx --output claude_results/
    python3 claude_coder.py scraped_content/ tracker.xlsx --limit 5  # Test on 5
    python3 claude_coder.py scraped_content/ tracker.xlsx --dry-run  # Preview
        """
    )
    ...
    parser.add_argument('--output', '-o', default='claude_results', help='Output directory')
    ...
    parser.add_argument('--max-content', type=int, default=None,
                        help='Override max content chars (default: 200000). Use lower value for rate-limited API tiers.')
    ...
    if not args.dry_run:
        print(f"\nNext steps:")
        print(f"  1. Run ChatGPT coder for IRR comparison")
        print(f"  2. Run: python3 irr_calculator.py {args.output}/ chatgpt_results/")
